...
universe/price/FX/quality pipeline, and executes three scenarios:
...
Sorting & Tie-Breaking:
  - Deterministic sort key: (-score, -raw, ticker)
    where score is percentile rank for all5_market_percentile, or raw r63 for raw scenarios.
"""
...
def compute_diagnostics(
    ...
    # Forward return calculations per selection row
    fwd_returns = []
    sessions_held_list = []
    ...
    return fwd_df, fwd_summary_df, disp_df, summary
...
def main() -> None:
    ...
    all_assets = ref.dedupe_assets(
        [a for values in groups.values() for a in values]
    ...
    scenarios_spec = [
        ("base_raw", groups_available["base"], False),
        ("all5_raw", all5_pool, False),
        ("all5_market_percentile", all5_pool, True),
    ]
    ...
    for scenario_name, scenario_tickers, use_pct in scenarios_spec:
        ...
        returns, selections, metrics = run_strategy(
            ...
            market_by_ticker, sleeve_by_ticker, use_percentile=use_pct
        )
    ...
    universe_source_warnings = [
        msg for msg in ref_log_messages
        if any(w in msg.lower() for w in ["failed", "proxy", "too small"])
    ]
    (outdir / "reference_run.log").write_text("\n".join(ref_log_messages) + "\n", encoding="utf-8")

    # Compute SHA-256 hashes of generated CSV files
    csv_files = [
        ...
        "displacement_diagnostics.csv", "reference_run.log",
    ]
